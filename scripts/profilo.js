...
function fetchGiphy(q, page){
    let url="API/giphyAPI.php?q="+encodeURIComponent(q)+"&page="+page;
...
function gifConfirmed(event){
    ...
    fetch("API/modifyProfileImage.php", {method: 'post', body: formData}).then((risposta) => 
    {
        if(risposta.ok)
...
function modificaPassword(event){
    ...
    fetch("API/modifyPassword.php", {method: 'post', body: formData}).then((risposta) => 
    {
        if(risposta.ok)
            return risposta.text();
    }).then((text)=>{
